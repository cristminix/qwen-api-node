...
import { ChatCompletionRequest, ChatResponse, ChatMessage, ChatResponseStream } from './core/types/chat';
import { Tool } from './core/types/tools';
import { ACTION_SELECTION_PROMPT, TOOL_GENERATION_PROMPT } from './core/prompts';
...
interface InternalPayloadMessage {
    ...
    content: string;
    chat_type: string;
    feature_config: { thinking_enabled: boolean; thinking_budget: number; output_schema: null };
    extra: {};
}
...
export class QwenAPI {
    ...
    constructor(authToken: string, cookie: string, baseURL: string = 'https://chat.qwen.ai') {
    ...
    public async uploadFile(filePath: string): Promise<UploadResult> {
        ...
        if (uploadResult.res.status !== 200) {
            throw new Error(`Failed to upload file to OSS. Status: ${uploadResult.res.status}`);
        }
    ...
    public async create(request: ChatCompletionRequest): Promise<ChatResponse> {
        if (request.tools && request.tools.length > 0) {
            ...
            const shouldUseToolResponse = await this._getRawChatResponse({ ...request, messages: actionSelectionMessages });
            ...
            if (shouldUseToolResponse.toLowerCase().includes('yes')) {
                ...
                const toolResponse = await this._getRawChatResponse({ ...request, messages: toolGenerationMessages });
                ...
                try {
                    ...
                    console.warn("Tool generation response was not valid JSON, falling back to regular chat.");
                }
            }
        }
    ...
    public async *stream(request: ChatCompletionRequest): AsyncGenerator<ChatResponseStream> {
        if (request.tools && request.tools.length > 0) {
    ...
    private async _getRawChatResponse(request: ChatCompletionRequest): Promise<string> {
    ...
    private async *_makeApiCallStream(request: ChatCompletionRequest): AsyncGenerator<ChatResponseStream> {
        try {
            const payload = {
                model: request.model,
                messages: request.messages.map(msg => {
                    ...
                    return {
                        ...
                        chat_type: "t2t",
                        feature_config: { thinking_enabled: false, thinking_budget: 0, output_schema: null },
                        extra: {}
            ...
            if (axios.isAxiosError(error)) {
                console.error("Axios error in _makeApiCallStream:", error.response?.data);
            } else {
    ...
    private async _makeApiCallNonStream(request: ChatCompletionRequest): Promise<ChatResponse> {
        try {
            const payload = {
                model: request.model,
                messages: request.messages.map(msg => {
            ...
            if (axios.isAxiosError(error)) {
                console.error("Axios error in _makeApiCallNonStream:", error.response?.data);
            } else {
